...
## Nachtrag 2 (`/refine PROJ-3`, 2026-08-28)

Die in diesem Dokument (und in den historischen Beispieldateien wie `20260820_demo_Sammelkartenspiel-OffSeason_Journey-Transkript.md`/`…_Konzept.md`) noch als zwei getrennte Dateien beschriebene Import-Werkstatt-Eingabe wurde per `/refine PROJ-3` auf **eine kombinierte Datei** umgestellt (Journey-Transkript-Block + Landingpage-Konzept-Block in einem Dokument, bewusster Breaking Change gegenüber dem alten Zwei-Datei-Format). Siehe `docs/reference/Interview-Import-Vorlage.md` für das neue kombinierte Format und `features/PROJ-3-import-werkstatt.md` für die vollständige Begründung. Die historischen Beschreibungen in diesem Dokument bleiben als Beleg für die ursprüngliche Herleitung des dreischichtigen Graph-Modells unverändert stehen.
