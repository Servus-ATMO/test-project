...
- [x] ~~**Wichtig für Produktion:** Supabase Auth → URL Configuration: `https://test-project-woad-theta.vercel.app/**` muss zu den erlaubten Redirect URLs hinzugefügt werden~~ — am 2026-08-28 erledigt (siehe Deployment-Abschnitt „Nachträglich erledigt"), inkl. eines dabei entdeckten zusätzlichen Vercel-SSO-Problems auf der zweiten Domain-Alias
...
**Nachträglich erledigt (2026-08-28, ausgelöst durch echten Login-/Reset-Fehlschlag des Nutzers in Produktion):**
- **Supabase Auth → URL Configuration korrigiert:** Site URL auf `https://test-project-woad-theta.vercel.app` gesetzt, `https://test-project-woad-theta.vercel.app/**` UND `http://localhost:3000/**` zu den erlaubten Redirect URLs hinzugefügt (Letzteres war zwischenzeitlich offenbar herausgefallen — auch der lokale PROJ-2-Regressionstest schlug dadurch kurzzeitig fehl, siehe unten). Per Supabase-Admin-API (`generateLink`) für beide Redirect-Ziele verifiziert, dass `redirect_to` jetzt korrekt dem angeforderten Wert entspricht statt auf die Site-URL-Fallback umzuspringen.
- **Eigener Fehler bei der ersten Korrektur, dabei entdeckt:** Zunächst wurde die Site URL fälschlich auf `https://test-project-atmodesign.vercel.app` gesetzt (die andere von zwei existierenden Domain-Aliasen derselben Produktions-Deployment). Diese Domain steht jedoch hinter **Vercels eigener Deployment-Protection/SSO** (`302` → `vercel.com/sso-api`, setzt `_vercel_sso_nonce`), nicht hinter der App-eigenen Login-Seite — für jeden ohne Zugriff auf das Vercel-Team `atmodesign` wäre die Seite darüber gar nicht erreichbar gewesen, unabhängig vom App-Login. `https://test-project-woad-theta.vercel.app` (von Vercel selbst als „Latest Production URL" geführt) ist dagegen öffentlich erreichbar (`307` → App-eigenes `/login`) und bleibt die einzige kanonische Produktions-URL für dieses Projekt (Dokumentation, Tests, Supabase-Config) — `atmodesign` bewusst nicht weiter verwendet.
- Fix per echtem PROJ-2-Regressionstest verifiziert (`full password-reset flow: real recovery link sets a new password that then works`) — schlug mit der falschen (atmodesign-)Site-URL fehl (Recovery-Link landete auf Vercels SSO-Seite statt `/passwort-zuruecksetzen`), lief nach der Korrektur wieder grün.
