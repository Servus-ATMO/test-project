...
- **Ein-Datei-Umstellung implementiert (`/backend`, 2026-08-28):**
  - **Datenbank:** Migration `interview_imports_single_file` — `journey_file_path`/`konzept_file_path` (zwei Spalten) ersetzt durch `raw_file_path` (eine Spalte). Der einzige bestehende Datensatz ("1. Testkunde") wurde best-effort auf seinen bisherigen Journey-Pfad migriert (rein historischer Wert, wird beim nächsten Re-Import überschrieben) — seine bereits geparsten `import_sections`/`import_entries`/`import_fields` blieben unangetastet, wie in der Product Decision festgelegt. `save_interview_import()` (Postgres-Funktion) auf die neue Spalte umgestellt (`p_raw_file_path` statt zwei Pfad-Parametern).
  - **Sicherheitsfund während der Migration (selbst behoben, kein QA-Bug):** Das Neuanlegen der Funktion per `DROP FUNCTION` + `CREATE OR REPLACE FUNCTION` hat den zuvor bewusst gesetzten `REVOKE EXECUTE ... FROM PUBLIC` aus der ursprünglichen `save_interview_import_atomic`-Migration zurückgesetzt (Postgres-Standardverhalten: neue Funktionen bekommen implizit `EXECUTE` für `PUBLIC`, wovon `anon` erbt). Direkt danach per `has_function_privilege` bemerkt und mit einer zusätzlichen Migration (`save_interview_import_revoke_public_execute`) korrigiert — verifiziert: `anon` kann die Funktion nicht mehr ausführen, `authenticated` weiterhin schon (deckt sich wieder mit dem von QA in der ersten Runde geprüften Zustand).
  - **Aufteilungs-Logik:** neue Datei `src/lib/imports/split-combined-import.ts` (`splitCombinedImport()`) zerlegt die eine hochgeladene Datei anhand der beiden bekannten H1-Überschriften (`# Journey-Transkript`, `# Landingpage-Konzept`) in zwei Rohtext-Blöcke, reihenfolgeunabhängig — exakt wie im Tech Design beschrieben, kein neues Trennzeichen. Die bestehenden `parseJourney()`/`parseKonzept()`-Funktionen bleiben dadurch vollständig unverändert. 5 neue Unit-Tests (`split-combined-import.test.ts`), inkl. vertauschter Reihenfolge und beider Fehlender-Block-Fälle.
  - **`format-detect.ts` bereinigt:** `detectDocumentType()`/`checkCrossFormat()` entfernt (siehe Tech Design — das „falscher Slot"-Problem kann mit nur einem Upload-Slot nicht mehr auftreten), `validateFile()`/`MAX_FILE_SIZE_BYTES` unverändert übernommen. `FormatWarning`-Typ durch `MissingBlock` (`'journey' | 'konzept' | null`) ersetzt.
  - **Server Actions (`src/lib/imports/actions.ts`):** `checkImportFiles(rawText: string)` und `saveImport(clientId, projectId, rawText, acknowledgeDependentData)` jetzt mit einem einzigen Rohtext-Parameter statt zwei. Hard-Fail („keine erkennbare Struktur") nur noch, wenn **beide** Block-Überschriften fehlen; fehlt genau ein Block, wird das als `missingBlock` in der Vorschau zurückgegeben statt hart abzulehnen (neue AC). Rohdatei-Upload jetzt unter `{projectId}/interview-import.md` (ein Objekt statt zwei).
  - **Frontend (`ImportPanel`, `/backend`-Mitzug):** ein `UploadZone` statt zwei, ein Datei-State statt zwei. Die alte Kreuz-Format-Warnung („Trotzdem fortfahren") ist jetzt eine Fehlender-Block-Warnung mit identischem Gating-Muster (Konfirmieren-Button erst nach explizitem „Trotzdem fortfahren" sichtbar) — direkte Wiederverwendung des bereits etablierten UX-Musters, wie im Tech Design vorgesehen. `upload-zone.tsx` war bereits generisch genug und musste nicht geändert werden.
  - **Verifikation:** `npm test` (108/108, davon 9 neu), `npm run lint`, `npm run build` sauber. Zusätzlich ein temporärer, nicht committeter Playwright-Smoke-Test gegen den echten lokalen Dev-Server + dieselbe (auch produktiv genutzte) Supabase-Instanz geschrieben und ausgeführt (danach wieder entfernt, kein permanenter Testfile): vollständiger Upload→Vorschau→Bestätigen-Durchlauf mit einer echten kombinierten Datei, `raw_file_path` und alle vier Tabellen korrekt befüllt, Rohdatei unter dem neuen Pfad abrufbar, alte Zwei-Datei-Pfade nachweislich nicht mehr vorhanden; zweiter Durchlauf mit einer Journey-only-Datei bestätigt die Fehlender-Block-Warnung inkl. Gating.
  - **Bewusst NICHT in dieser Runde behoben:** Die permanente E2E-Suite `tests/PROJ-3-import-werkstatt.spec.ts` ist größtenteils auf das alte Zwei-Upload-Feld-UI zugeschnitten (`input[type="file"] >> nth=1`, Kreuz-Format-Warnungstexte, zwei Storage-Pfade) und wird gegen die neue UI mehrheitlich fehlschlagen. Nur die beiden direkten DB-/RPC-Aufrufe, die exakt an die Spaltenumbenennung gekoppelt waren (RLS-Schreibversuch, Atomizitäts-Regressionstest), wurden mechanisch angepasst, damit sie überhaupt gegen das neue Schema laufen. Das vollständige Neuschreiben der browsergetriebenen Upload-Flows (neue Fixture als eine kombinierte Datei, neue Assertions für die Fehlender-Block-Warnung, neuer Storage-Pfad) ist bewusst `/qa PROJ-3` überlassen, konsistent mit der Aufgabentrennung des Projekts (QA schreibt/pflegt die permanente E2E-Suite).
...
| **[Bugfix]** Strukturierter Speichervorgang (`interview_imports`/`import_sections`/`import_entries`/`import_fields`) in eine einzige Postgres-Funktion (`save_interview_import`, `SECURITY INVOKER`) verschoben, statt mehrerer einzelner Client-Inserts | QA-BUG-1: die vorherige Implementierung war nicht transaktional, ein Fehler mitten im Vorgang konnte Teildaten dauerhaft in der DB hinterlassen. Eine Postgres-Funktion ist implizit transaktional — schlägt irgendein Schritt fehl, rollt der gesamte Aufruf automatisch zurück. `SECURITY INVOKER` (nicht `DEFINER`) bewusst gewählt, damit RLS/GRANTs weiterhin exakt wie bei einzelnen Aufrufen greifen, keine Rechteausweitung | 2026-08-27 |
| Rohdatei-Upload in den `imports`-Bucket erst NACH dem erfolgreichen DB-Schreiben, nicht mehr davor | QA-BUG-2: bei paralleler Upload-Reihenfolge (vorher) konnte ein Teilfehlschlag (nur eine der beiden Dateien hochgeladen) ein verwaistes Storage-Objekt hinterlassen, ohne dass je eine DB-Zeile darauf verwies. Schlägt der DB-Schritt jetzt fehl, wird gar nichts hochgeladen; schlägt nur der Upload fehl, sind die strukturierten (tatsächlich genutzten) Daten bereits vollständig gespeichert — dafür ein neuer nicht-blockierender `storage-warning`-Status statt eines harten Fehlers | 2026-08-27 |
...
- **Status: FIXED (2026-08-27, `/backend`-Bugfix-Runde).** `saveImport()` schreibt die strukturierten Daten jetzt über die neue Postgres-Funktion `save_interview_import()` (Migration `save_interview_import_atomic`) — ein einziger, implizit transaktionaler Funktionsaufruf statt fünf separater Inserts. Empirisch verifiziert: ein gezielter Fehlversuch direkt gegen die Funktion (Fremdschlüssel-Verletzung bei `import_entries`, *nachdem* `import_sections` innerhalb desselben Aufrufs bereits geschrieben wurde) hinterlässt nachweislich null Zeilen in `interview_imports` und `import_sections` — anders als beim ursprünglichen Code, bei dem genau dieses Szenario Teildaten übrig gelassen hätte. Permanenter Regressionstest: `tests/PROJ-3-import-werkstatt.spec.ts` → „save_interview_import() is atomic…“. Vollständige Regression (20/20 Playwright, 70/70 Vitest) weiterhin grün.
