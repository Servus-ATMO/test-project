...
- **BUG-10 (echte Kundendaten, 2026-08-28):** 12 von 38 gefundenen Dimensionswerten bei "1. Testkunde" hatten `informs`-, aber keine `shapes`-Kanten (per SQL bestätigt) — u. a. "Business Goal", "Desire", "Emotional Drivers", "User Intent", konzentriert auf eher strategische/indirekte Dimensionen. Zwei Ursachen identifiziert: (1) Prompt-Formulierung in Schritt 3 war Content-Block-zentrisch ("ordne jedem Content-Block die Dimensionen zu"), ohne Anforderung, dass umgekehrt jede Dimension mindestens eine ausgehende Kante braucht — die KI ließ strategische Dimensionen ohne klaren Einzel-Block-Bezug dadurch plausibel ohne Kante. (2) Echter Parser-Bug: anders als bei "Quelle" wurde eine "Kante:"-Überschrift, die nicht exakt ins erwartete Muster passte, in `parseShapesEdges()` bisher komplett stillschweigend übersprungen (`if (!match) continue`, keine Warnung, keine Kante). **Fix (dreigleisig, auf Nutzerwunsch Prävention statt nur Sichtbarkeit):** (1) `prompt-template.ts`/`KI-Anreicherungs-Ergebnis-Vorlage.md`: explizite Anforderung ergänzt, dass jede nicht-"nicht ableitbar"-Dimension mindestens eine Kante braucht (auch strategische, dann typischerweise zum Hero/stärksten Conversion-Block), plus explizites Überschriftenformat mit Warnung vor Abweichungen. (2) `KANTE_HEADING`-Regex in `parse-enrichment.ts` toleriert jetzt eine fehlende "Kante:"-Präfix-Angabe. (3) Sicherheitsnetz: eine Überschrift, die trotzdem gar keinem Muster entspricht, landet jetzt als Warnung in `unresolvedReferences` statt spurlos zu verschwinden. 2 neue Unit-Tests. **Wichtig:** Wirkt nur auf künftige Durchläufe; die bereits gespeicherte "1. Testkunde"-Anreicherung bleibt unverändert, bis erneut hochgeladen wird — und selbst dann bleibt offen, ob die KI beim nächsten Durchlauf tatsächlich für jede Dimension eine plausible Kante findet (Prompt-Anforderung ist eine Empfehlung an die KI, keine Garantie).
- **BUG-7 (UX-Feedback, 2026-08-28):** Nutzer verwechselte eine "Quelle konnte nicht zugeordnet werden"-Warnung für eine Dimension (z. B. "Desire") mit der daneben korrekt angezeigten Instanz derselben Dimension für eine andere Persona — nachvollziehbar, da die Warnmeldung bisher nur den Dimensionsnamen nannte, nicht die betroffene Persona. Eine Dimension hat aber pro Persona eine eigene Werte-Instanz; die Warnung galt einer anderen (tatsächlich nicht-anwendbaren) Instanz als der sichtbar korrekten. **Fix:** Warnmeldung in `parseDimensions()` (`parse-enrichment.ts`) nennt jetzt zusätzlich die Persona, z. B. `Dimension "Desire" (Persona: Investoren): Quelle "entfällt" …`. 1 neuer Unit-Test.
- **BUG-6 (echte Kundendaten, 2026-08-28):** Nach dem BUG-5-Fix blieben einige „Quelle:"-Referenzen trotz korrektem `[Eintrag-Label] → [Feldname]`-Format weiterhin unaufgelöst, z. B. `Notizen zur Aufnahme → Übersprungene oder zusätzliche Fragen`. Ursache: Sections mit genau einem unbenannten Eintrag (`Einstieg`, `Notizen zur Aufnahme` in Journey-Dokumenten, mehrere Konzept-Abschnitte wie „1. Kurzfassung") haben ein leeres `entry.label` — `renderSection()` in `prompt-template.ts` zeigt für einen solchen Eintrag dementsprechend KEINE `#### [Eintrag-Label]`-Zeile, die KI sieht im eingebetteten Prompt-Text nur die Section-Überschrift und zitiert diese folgerichtig als Eintrag-Label. `buildFieldLookup()` in `parse-enrichment.ts` indizierte diese Felder bisher aber unter dem leeren `entry.label`, nicht unter dem Section-Namen — ein garantierter Mismatch für jede Referenz auf ein Feld einer solchen Section. **Fix:** `buildFieldLookup()` verwendet jetzt `entry.label || section.name` als Lookup-Schlüssel, deckungsgleich mit dem, was der Prompt der KI tatsächlich zeigt. 1 neuer Unit-Test (`parse-enrichment.test.ts`).
- **BUG-5 (echte Kundendaten, 2026-08-28, kritisch):** Beim ersten vollständigen Durchlauf für "1. Testkunde" lösten sich 0 von 38 "Quelle:"-Referenzen auf (`informs`-Kanten fehlten komplett, per SQL gegen `enrichment_edges` bestätigt) — jeder Dimensionswert verlor dadurch seine Quell-Verknüpfung, seinen Impact-Text und seine Gewichtung. Ursache: `parse-enrichment.ts`s `splitReference()` erwartete strikt genau ein `[Eintrag-Label] → [Feldname]`-Paar; die KI hängte an den Feldnamen aber häufig einen Klammerzusatz an (z. B. `Antwort (Option D)`) und kombinierte mehrere Quellen mit Semikolon in einer Zeile (z. B. `Frage 1 → Antwort (Option D); Frage 2 → Antwort (Option A)`) — beides ließ die exakte Lookup-Übereinstimmung gegen die importierten Feldnamen fehlschlagen. **Fix (zweigleisig):** (1) `resolveFieldIds()` ersetzt `splitReference()` — splittet zuerst an Semikolon, prüft pro Teil zuerst exakt, dann zusätzlich ohne einen angehängten Klammerzusatz, und erzeugt bei mehreren auflösbaren Quellen mehrere `informs`-Kanten statt nur einer; echte Freitext-Quellen ohne "→" (z. B. "entfällt, …") bleiben bewusst unaufgelöst und landen als Warnung, nicht als geratene Zuordnung. (2) `prompt-template.ts`/`KI-Anreicherungs-Ergebnis-Vorlage.md` fordern jetzt explizit genau ein Referenz-Paar ohne Klammerzusatz und ohne Semikolon-Kombination an (mit Negativbeispiel), um die Häufigkeit des Problems bei künftigen Durchläufen zu senken. 4 neue Unit-Tests (`parse-enrichment.test.ts`). **Wichtig:** Die bereits gespeicherte "1. Testkunde"-Anreicherung ist von diesem Fix nicht automatisch betroffen — ein erneuter Upload derselben (oder einer neuen) Ergebnisdatei ist nötig, um die Kanten nachträglich aufzulösen.
...
**Hinweis zur Build-Verifikation:** Der Vercel-CLI-Zugriff war in dieser Sitzung durch die Auto-Mode-Berechtigungen blockiert (kein Vercel-Token verfügbar) — die Verifikation erfolgte ausschließlich über direkte HTTP-Prüfungen der Produktions-URL nach einer Wartezeit für den automatischen GitHub-Push-Deploy, nicht über eine direkte Prüfung des Vercel-Build-Logs selbst. Der `vercel`-CLI-Zugriff wurde danach für lesende Befehle (`ls`/`inspect`/`logs`) freigegeben, siehe `.claude/settings.json`.

**Nachträglich erledigt (2026-08-28, Nutzer-Feedback nach dem Deploy):**
- **BUG-2: Bereits erzeugter Anreicherungs-Prompt ging beim Verlassen/erneuten Besuch der Projekt-Seite verloren.** War reiner React-Client-State, nirgends persistiert. Da der Workflow (Prompt kopieren → in einem separaten Claude-Chat ausführen → später zurückkommen) sich naturgemäß über mehrere Seitenbesuche erstreckt, war das spürbar unpraktisch, obwohl fachlich kein Datenverlust vorlag (der Prompt ist jederzeit identisch aus den bereits importierten Daten neu erzeugbar, siehe Edge-Case-Dokumentation oben). **Severity:** Low/UX (kein AC verletzt). **Fix:** Prompt wird jetzt zusätzlich in `localStorage` gespeichert (Schlüssel pro Projekt-ID), beim erneuten Besuch automatisch wiederhergestellt, beim Abbrechen oder erfolgreichen Speichern wieder entfernt. Bewusst kein Server-/DB-Persistenz gewählt, da der Prompt reine Ableitungsdaten ohne Informationsgehalt über das Original hinaus sind. Neuer Playwright-Regressionstest (Reload behält den Prompt) in `tests/PROJ-4-ki-anreicherung.spec.ts` ergänzt, volle Regression (25/25 Playwright, 75/75 Vitest) weiterhin grün.
- Im Zuge derselben Nutzer-Rückmeldung wurde außerdem ein produktionsweites Supabase-Auth-Konfigurationsproblem gefunden und behoben (falsche/fehlende Redirect-URLs, plus eine fälschlich verwendete, Vercel-SSO-geschützte Domain-Alias) — betrifft PROJ-2 (Login/Passwort-Reset), nicht PROJ-4 selbst; vollständig dokumentiert in `features/PROJ-2-agentur-login.md` („Nachträglich erledigt", 2026-08-28).
- **BUG-4-, BUG-5- und BUG-6-Fixes deployt (2026-08-28).** Commits `c386e84` (Prompt weist zur Ausgabe als Datei/Artifact an), `15b689e` (Quelle-Referenzen mit Klammerzusatz/Semikolon werden aufgelöst) und `c7d9353` (Quelle-Referenzen auf unbenannte Einzel-Eintrag-Sections werden aufgelöst) gepusht, jeweils Vercel-Build erfolgreich (Status Ready), `/login` liefert 200 auf `test-project-woad-theta.vercel.app`. Details siehe BUG-4/BUG-5/BUG-6 in den Implementierungsnotizen oben. Bereits gespeicherte Anreicherungen (z. B. "1. Testkunde") sind davon nicht automatisch betroffen — erneuter Upload der Ergebnisdatei nötig, um weitere Referenzen aufzulösen.
