...
- **BUG-4- und BUG-5-Fixes deployt (2026-08-28).** Commits `c386e84` (Prompt weist zur Ausgabe als Datei/Artifact an) und `15b689e` (Quelle-Referenzen mit Klammerzusatz/Semikolon werden aufgelöst) gepusht, Vercel-Build erfolgreich (Status Ready), `/login` liefert 200 auf `test-project-woad-theta.vercel.app`. Details siehe BUG-4/BUG-5 in den Implementierungsnotizen oben. Bereits gespeicherte Anreicherungen (z. B. "1. Testkunde", 0/38 `informs`-Kanten) sind davon nicht automatisch betroffen — erneuter Upload der Ergebnisdatei nötig, um die Kanten nachträglich aufzulösen.
