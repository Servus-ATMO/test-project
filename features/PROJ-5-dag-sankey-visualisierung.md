...
- **Bugfix + neue Anforderung nach Nutzer-Feedback (`/frontend`, 2026-08-29):**
  - **Bug: Persona-Filter sprang beim Anklicken eines Knotens auf „Alle Personas" zurück.** Ursache: `handleNodeSelect()` setzte bisher bewusst `setSelectedPersona(null)`, weil Knoten-Auswahl und Persona-Filter als sich gegenseitig ausschließende Highlight-Quellen entworfen waren. Nutzer-Erwartung ist aber, dass das Dossier unabhängig vom aktiven Filter öffnet/schließt und der Filter (inkl. hervorgehobener Kanten/Knoten) dabei bestehen bleibt. Fix: `handleNodeSelect()` rührt `selectedPersona` nicht mehr an; die Highlight-Berechnung in `renderData` priorisiert jetzt `selectedPersona` vor `selectedNode` (vorher umgekehrt) — ist ein Filter aktiv, bestimmt er weiterhin das Highlight. `selectedPersona` wird weiterhin explizit zurückgesetzt beim Ausblenden von Ebene 2 (`handleEbene2Toggle`) und beim bewussten Wechsel auf eine andere/keine Persona (`handlePersonaChange`) — beides eigene, gewollte Nutzerhandlungen, kein Bug.
  - **Direkter Folge-Bug, noch in derselben Runde gefunden:** Ein Knoten *außerhalb* der aktiven Persona blieb entgegen der Filter-Regel voll sichtbar (statt ausgegraut), sobald sein Dossier geöffnet wurde — verschwand aber wieder korrekt beim Schließen. Ursache: `highlightFor()`s „selected"-Sonderfall (`if (selectedNode && nodeId === selectedNode.id) return 'selected'`) griff bedingungslos, unabhängig davon, ob der Persona-Filter diesen Knoten überhaupt als aktiv einstuft — der geklickte Knoten wurde dadurch immer „freigestellt", auch wenn er inhaltlich gar nicht zur gefilterten Persona gehört. Fix: der „selected"-Ring gilt jetzt nur noch, wenn die Knoten-Auswahl selbst die Highlight-Quelle ist (`!selectedPersona && ...`) — ist ein Persona-Filter aktiv, folgt auch der angeklickte Knoten wie jeder andere der normalen aktiv/ausgegraut-Regel des Filters, das Dossier ändert daran nichts.
